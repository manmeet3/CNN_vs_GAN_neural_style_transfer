...
def make_generator_model(n_blocks):    
    ...
    model.add(tf.keras.layers.Conv2D(3, (7, 7), strides=tuple(ti/2 for ti in stride_size), padding='valid', activation='tanh'))
...
def make_discriminator_model():
    ...
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.Conv2D(64, (4, 4), strides=stride_size, padding=pad, input_shape=(256, 256, 3)))
    model.add(tf.keras.layers.LeakyReLU(alpha=0.2))

    model.add(tf.keras.layers.Conv2D(128, (4, 4), strides=stride_size, padding=pad, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(alpha=0.2))

    model.add(tf.keras.layers.Conv2D(256, (4, 4), strides=stride_size, padding=pad, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(alpha=0.2))

    model.add(tf.keras.layers.Conv2D(512, (4, 4), strides=tuple(ti/2 for ti in stride_size), padding=pad, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(alpha=0.2))

    model.add(tf.keras.layers.Conv2D(1, (4, 4), strides=tuple(ti/2 for ti in stride_size), padding=pad))
